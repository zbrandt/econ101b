...
\begin{homeworkProblem}
    ...
    only $G$, $A$, and $\psi$. (Hint: Recall that $N=1$.)
    \\ \\ 
    \solution
    ...
    into the resource constraint $Y = C + G$ to find output in terms of only $G$, $A$, and $\psi$.
    ...
    \end{minipage}
    \\ \\ \\
    From the right hand side we see that $Y = \frac{A + \psi G}{\psi + 1}$.
    On the left hand side we solved for hours worked and then substituted this expression into our
    resource constraint equation to find output in terms of $G$, $A$, and $\psi$. 
