...
\begin{homeworkProblem}[1]{Recovering from a War}
    ...
    Now, to find the time series plots of $Y_t$ and $K_t$ we need to solve the differential equation $\frac{dK}{dt} = sAK^{1/3}L^{2/3} - dK$. General solutions to this nonlinear differential equation take the form
    \[
        \begin{split}
            K(t) = \overline{K} - \left( K_0 - \overline{K} \right)e^{-dt}
        \end{split}
    \]
    and subsituting in our expression for $\overline{K}$ our plots for output and capital are
    \[
        \begin{split}
            ...
            \\
            Y(t) &= \overline{A} (K(t))^{1/3} \overline{L}^{2/3} 
        \end{split}
    \]
